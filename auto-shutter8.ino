...
#define cmdSetNumber "Set Number"
...
void setup() {
  ...
  // IMPORTANT: Please comment out the line below to use the gsm for the first time
  // setGsmConfig();
  ...
  if (sim.available() > 0) {
     ...
     if (msg.indexOf("+CMT:") != -1) {
       ...
       if (phn_num.indexOf("+880") != -1) {
        ...
        } else if(msg_cmd == cmdSetNumber) {
            setNumber();
            SendMessage(phn_num, "Number saved successfully");
         } else {
...
void setNumber() {
  ...
  delay(50);
  String cmd = "AT+CPBW=,\"" + BUILT_IN_CONTACT_NUMBER + "\",129,\"" + BUILT_IN_CONTACT_NAME + "\"";
  sim.println(cmd); // save contact to SIM
  delay(50);
}
...
void checkNumber() {
  while (1) {
    ...
    if (str.indexOf("+CPBF:")!=-1) {
      ...
      } else if (EEPROM.read(0) == 0) { // if already suspended
